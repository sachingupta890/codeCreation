...
void selectionSort(Node*&head,Node*&tail){
    ...
    while(outerLoop != tail){
        ...
        swap(minNode->data,outerLoop->data);
...
void bubbleSort(Node*&head,Node*&tail){
    Node* outerLoop = head;
    Node* innerLoop;
    int count = totalNode(head);
    while(outerLoop -> next!= NULL){
        innerLoop = head;
        
        count = count-1;
        while(innerLoop->next!= NULL&& count != 0){
            if(innerLoop->next->data < innerLoop->data){
                swap(innerLoop->next->data,innerLoop->data);
            }
            innerLoop=innerLoop->next;
            count --;
        }
